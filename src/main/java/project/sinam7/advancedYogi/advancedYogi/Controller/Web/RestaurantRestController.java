...
public class RestaurantRestController {
    ...
    public List<Restaurant> getRestaurants(@ModelAttribute SearchFilter searchFilter, @PathVariable(required = false) String pageNum) {
        int num = pageNum.isEmpty() ? 0 : Math.max(Integer.parseInt(pageNum), 0);

        return restaurantService.getRestaurants(searchFilter.getLatitude(), searchFilter.getLongitude(), num);
    }
